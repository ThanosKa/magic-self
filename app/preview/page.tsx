...
async function ProcessPreview() {
  ...
  if (!resumeData) {
    ...
    aiLogger.info({ userId, usedFallback: result.usedFallback }, "Resume data saved")
  }
  ...
  if (!username && user) {
    ...
    const baseName = user.firstName
      ? generateUsernameFromName(`${user.firstName} ${user.lastName || ""}`.trim())
      : "user"
  ...
  return <PreviewClient resumeData={resumeData} username={username} showFallbackTip={showFallbackTip} />
}
...
function LoadingFallback() {
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-4 p-6">
      ...
      <p className="text-lg text-muted-foreground">Creating your personal website...</p>
    </main>
  )
}
