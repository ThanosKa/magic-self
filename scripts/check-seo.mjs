...
async function checkRobotsTxt() {
  ...
  try {
    ...
    const checks = [
      ...
      {
        test: content.includes("Allow:") || content.includes("Disallow:"),
        msg: "Crawl directives found",
      },
    ];
...
async function checkSitemap() {
  ...
  try {
    ...
    const checks = [
      ...
      {
        test: content.includes("<lastmod>"),
        msg: "Last modified dates present",
      },
    ];
  ...
  try {
    ...
    const checks = [
      {
        ...
        msg: "Page title present",
        extract: /<title>(.+?)<\/title>/,
      },
      {
        test: /<meta name="description"/.test(html),
        msg: "Meta description present",
        extract: /<meta name="description" content="(.+?)"/,
      },
      {
        test: /<meta name="keywords"/.test(html),
        msg: "Meta keywords present",
      },
      {
        test: /<link rel="canonical"/.test(html),
        msg: "Canonical URL present",
      ...
      { test: /<meta property="og:title"/.test(html), msg: "OG title present" },
      {
        ...
        msg: "OG description present",
      },
      {
        test: /<meta property="og:image"/.test(html),
        msg: "OG image present",
      ...
      {
        test: /<meta name="twitter:title"/.test(html),
        msg: "Twitter title present",
      },
      {
        test: /<meta name="twitter:description"/.test(html),
    ...
    checks.forEach(({ test, msg, extract }) => {
      if (test) {
        ...
        if (extract) {
          ...
          if (match && match[1]) {
            log.info(
              `  → ${match[1].substring(0, 80)}${match[1].length > 80 ? "..." : ""}`
            );
          }
        }
      } else {
    ...
    if (titleMatch) {
      ...
      if (titleLength < 30 || titleLength > 60) {
        log.warn(
          `Title length (${titleLength}) should be 30-60 characters for optimal SEO`
        );
      }
    }
    ...
    if (descMatch) {
      ...
      if (descLength < 120 || descLength > 160) {
        log.warn(
          `Description length (${descLength}) should be 120-160 characters for optimal SEO`
        );
      }
    }
...
async function checkPerformanceHints() {
  ...
  try {
    ...
    const checks = [
      {
        test: /<link rel="preload"/.test(html),
        msg: "Resource preloading detected",
      },
      {
        test: /<link rel="preconnect"/.test(html),
        msg: "DNS preconnect hints",
      },
...
async function checkConfig() {
  ...
  try {
    ...
    const checks = [
      {
        test: /keywords:\s*\[/.test(configContent),
        msg: "SEO keywords defined",
      },
      { test: /ogImage:/.test(configContent), msg: "OG image path configured" },
      {
        test: /twitterHandle:/.test(configContent),
        msg: "Twitter handle configured",
      },
    ...
    checks.forEach(({ test, msg }) => {
      if (test) {
        ...
        log.warn(
          msg.replace("configured", "missing").replace("defined", "missing")
        );
      }
    });
