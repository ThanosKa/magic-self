...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!ALLOWED_FILE_TYPES.includes(file.type as (typeof ALLOWED_FILE_TYPES)[number])) {
      uploadLogger.warn({ userId: user.id, fileType: file.type }, "Invalid file type")
      return NextResponse.json({ error: "Only PDF files are allowed" }, { status: 400 })
    }

    // Validate file size
    if (file.size > MAX_FILE_SIZE) {
      uploadLogger.warn({ userId: user.id, fileSize: file.size }, "File too large")
    ...
    const { error: uploadError } = await supabase.storage.from("resumes").upload(fileName, buffer, {
      contentType: file.type,
      upsert: true,
    })

    if (uploadError) {
      uploadLogger.error({ userId: user.id, error: uploadError.message }, "Upload failed")
    ...
    uploadLogger.info({ userId: user.id, fileName: file.name }, "Upload successful")
    ...
    uploadLogger.error({ error: error instanceof Error ? error.message : "Unknown error" }, "Upload error")
