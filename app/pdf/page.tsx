...
async function ProcessPdf() {
  ...
  if (resume.file_content) {
    pdfLogger.info({ userId }, "PDF content already extracted, redirecting to preview")
    redirect("/preview")
  }

  try {
    // Extract text from PDF
    pdfLogger.info({ userId, fileUrl: resume.file_url }, "Extracting PDF content")
...
function LoadingFallback() {
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-4 p-6">
      ...
      <p className="text-lg text-muted-foreground">Reading your resume carefully...</p>
    </main>
  )
}
