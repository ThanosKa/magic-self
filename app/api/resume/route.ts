...
export async function GET() {
  try {
    ...
    dbLogger.error({ error: error instanceof Error ? error.message : "Unknown error" }, "Get resume error")
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (body.resumeData) {
      ...
      if (!result.success) {
        return NextResponse.json({ error: "Invalid resume data", details: result.error.flatten() }, { status: 400 })
      }
    ...
    dbLogger.error({ error: error instanceof Error ? error.message : "Unknown error" }, "Update resume error")
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    dbLogger.info({ userId: user.id, status: body.status }, "Resume status updated")

    return NextResponse.json({ resume })
  } catch (error) {
    dbLogger.error({ error: error instanceof Error ? error.message : "Unknown error" }, "Update status error")
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
